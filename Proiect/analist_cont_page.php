...
// Funcție pentru a adăuga în istoric utilizare
function adaugaIstoricUtilizare($conn, $actiune, $operatia_sql) {
    $utilizator = "analist";
    $rol = "analist";
    $data_ora = date('Y-m-d H:i:s');
    
    $stmt = $conn->prepare("INSERT INTO istoric_utilizare (utilizator, rol, data_ora, actiune, operatia_sql) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("sssss", $utilizator, $rol, $data_ora, $actiune, $operatia_sql);
    $stmt->execute();
    $stmt->close();
}
...
if (isset($_POST['cauta'])) {
    ...
    // Construim query-ul pentru istoric
    $sql_istoric = "SELECT * FROM contestatii WHERE 1=1";
    if (!empty($id)) $sql_istoric .= " AND id = " . $conn->real_escape_string($id);
    if (!empty($nume)) $sql_istoric .= " AND nume LIKE '%" . $conn->real_escape_string($nume) . "%'";
    
    // Adăugăm în istoric
    adaugaIstoricUtilizare($conn, "Căutare elevi", $sql_istoric);
}
...
if (in_array($sortColumn, $validColumns)) {
    ...
    // Adăugăm în istoric operația de sortare
    $sql_istoric_sortare = $sql;
    adaugaIstoricUtilizare($conn, "Sortare " . ($sortOrder === 'asc' ? 'ascendentă' : 'descendentă') . " după " . $sortColumn, $sql_istoric_sortare);
} else {
    $sql .= " ORDER BY id ASC";
}
...
if (isset($_GET['actiune']) && $_GET['actiune'] == 'selecteaza' && isset($_GET['id'])) {
    ...
    if ($row = $selectResult->fetch_assoc()) {
        ...
        // Adăugăm în istoric
        $sql_istoric = "SELECT * FROM contestatii WHERE id = $selectId";
        adaugaIstoricUtilizare($conn, "Selectare elev pentru vizualizare", $sql_istoric);
    }
