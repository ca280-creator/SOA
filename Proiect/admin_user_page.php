...
// Funcție pentru a adăuga în istoric
function adaugaInIstoricUtilizare($conn, $utilizator, $rol, $actiune, $operatia_sql) {
    $stmt = $conn->prepare("INSERT INTO istoric_utilizare (utilizator, rol, data_ora, actiune, operatia_sql) VALUES (?, ?, NOW(), ?, ?)");
    $stmt->bind_param("ssss", $utilizator, $rol, $actiune, $operatia_sql);
    $stmt->execute();
    $stmt->close();
}
...
if (isset($_POST['cauta'])) {
    ...
    // Adăugăm în istoric
    $operatia_sql = "SELECT * FROM users WHERE 1=1";
    if (!empty($id)) {
        $operatia_sql .= " AND id = " . $conn->real_escape_string($id);
    }
    if (!empty($username)) {
        $operatia_sql .= " AND username LIKE '%" . $conn->real_escape_string($username) . "%'";
    }
    if (!empty($password)) {
        $operatia_sql .= " AND password LIKE '%" . $conn->real_escape_string($password) . "%'";
    }
    if (!empty($role)) {
        $operatia_sql .= " AND role LIKE '%" . $conn->real_escape_string($role) . "%'";
    }
    
    adaugaInIstoricUtilizare($conn, "admin", "admin", "Căutare utilizatori", $operatia_sql);
}
...
if (isset($_POST['adauga'])) {
    ...
    if (empty($username)) {
        ...
        if ($stmt->execute()) {
            ...
            // Adăugăm în istoric
            $operatia_sql = "INSERT INTO users (username, password, role) VALUES ('" . $username . "', '" . $password . "', '" . $role . "')";
            adaugaInIstoricUtilizare($conn, "admin", "admin", "Adăugare utilizator nou", $operatia_sql);
...
if (isset($_POST['actualizeaza'])) {
    ...
    if (empty($id)) {
        ...
        if ($stmt->execute()) {
            ...
            // Adăugăm în istoric
            $operatia_sql = "UPDATE users SET username = '" . $username . "', password = '" . $password . "', role = '" . $role . "' WHERE id = " . $id;
            adaugaInIstoricUtilizare($conn, "admin", "admin", "Actualizare utilizator", $operatia_sql);
        } else {
...
if (isset($_POST['sterge'])) {
    ...
    if (empty($id)) {
        ...
        if ($stmt->execute()) {
            ...
            // Adăugăm în istoric
            $operatia_sql = "DELETE FROM users WHERE id = " . $id;
            adaugaInIstoricUtilizare($conn, "admin", "admin", "Ștergere utilizator", $operatia_sql);
...
if (isset($_GET['actiune']) && $_GET['actiune'] == 'selecteaza' && isset($_GET['id'])) {
    ...
    if ($row = $selectResult->fetch_assoc()) {
        ...
        // Adăugăm în istoric
        $operatia_sql = "SELECT * FROM users WHERE id = " . $selectId;
        adaugaInIstoricUtilizare($conn, "admin", "admin", "Selectare utilizator pentru editare", $operatia_sql);
    }
